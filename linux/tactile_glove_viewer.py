...
class ViewerApp:
    ...
    def start_watcher(self) -> None:
        ...
        print(f"[viewer] hot-plug watcher running (scan every {self.WATCH_INTERVAL}s) — "
              "plug in a glove (or close any app holding the serial port) and it goes live automatically")
        try:
            ...
            if ports:
                ...
                print("[viewer] NO serial ports detected. If a glove is plugged in but not "
                      "listed, check for /dev/ttyUSB* or /dev/ttyACM*, ensure the USB-serial "
                      "driver (e.g. CH340) is loaded, and that your user is in the 'dialout' "
    ...
    def _probe_sec_for_port(cls, dev: str) -> float:
        # USB-serial gloves usually stream immediately; BT / USB-CDC dongles lag after
        # plug-in. On Linux those enumerate as /dev/ttyACM* or /dev/rfcomm* (and as
        # /dev/cu.usbmodem-* on macOS), so give them the longer probe window.
        d = dev.lower()
        if "usbmodem" in d or "ttyacm" in d or "rfcomm" in d:
        ...
        for dev in sorted(present):
            ...
            if info.get("ok"):
                ...
                if info.get("binary_flood") or mode == "bt_dongle":
                    extra = " [BT dongle: RF may be paired but the AT+CONN bridge isn't active yet]"
...
def _check_assets() -> None:
    ...
    if missing:
        ...
        print("[viewer] Re-copy the full tactile-glove-demo-linux folder from the release zip.")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--left", default="", help="Force left glove serial port (e.g. /dev/ttyUSB0)")
    p.add_argument("--right", default="", help="Force right glove serial port (e.g. /dev/ttyUSB1)")
